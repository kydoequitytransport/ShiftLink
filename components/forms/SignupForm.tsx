      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <button
          ...
          onMouseEnter={(e) => {
            ...
            (e.currentTarget as HTMLElement).style.background = 'var(--color-sage-light)';
          }}
          onMouseLeave={(e) => {
        ...
        >
          ...
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--color-sage-dark)" strokeWidth="1.75" strokeLinecap="round">
              <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
              <circle cx="12" cy="7" r="4" />
            </svg>
          </div>
          <div>
            <p style={{ fontSize: '0.9375rem', fontWeight: 600, color: 'var(--color-ink)', marginBottom: '4px' }}>
              Healthcare Professional
            </p>
            <p style={{ fontSize: '0.8125rem', color: 'var(--color-ink-muted)', lineHeight: 1.6 }}>
              RN, LVN, CNA, Surgical Tech, and more — browse and claim open shifts.
            </p>
          </div>
          <svg style={{ marginLeft: 'auto', flexShrink: 0, color: 'var(--color-ink-faint)', marginTop: '4px' }} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
        ...
        {/* Facility card */}
        <button
          onClick={() => handleSelectType('facility')}
          style={{
            background: 'var(--color-white)',
            border: '1px solid var(--color-border)',
            borderRadius: 'var(--radius-md)',
        ...
        >
          ...
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--color-ink-muted)" strokeWidth="1.75" strokeLinecap="round">
          ...
          <div>
            ...
            <p style={{ fontSize: '0.8125rem', color: 'var(--color-ink-muted)', lineHeight: 1.6 }}>
              Hospital, clinic, or long-term care — post shifts and find coverage fast.
            </p>
          </div>
          <svg style={{ marginLeft: 'auto', flexShrink: 0, color: 'var(--color-ink-faint)', marginTop: '4px' }} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
            <path d="M9 18l6-6-6-6" />
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '4px' }}>
        <button
          ...
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-ink-muted)', fontSize: '0.8125rem', display: 'flex', alignItems: 'center', gap: '4px' }}
        >
          ← Back
        </button>
      ...
      {userType === 'professional' ? (
        <>
          ...
          <Select
            ...
            placeholder="Select your clinical role"
            onChange={(e) => setProForm((p) => ({ ...p, role: e.target.value as ShiftRole }))}
            error={errors.role}
          />
          <Input
            label="License Number"
            value={proForm.licenseNumber}
            onChange={(e) => setProForm((p) => ({ ...p, licenseNumber: e.target.value }))}
            placeholder="e.g. RN123456"
            error={errors.licenseNumber}
            hint="Your state license number — used for credential verification only."
          />
          <Input
            ...
            min="0"
            max="50"
            value={proForm.yearsExp}
            onChange={(e) => setProForm((p) => ({ ...p, yearsExp: e.target.value }))}
            placeholder="3"
          />
        </>
      ) : (
        <>
          <Input
            label="Facility Name"
            value={facForm.facilityName}
            onChange={(e) => setFacForm((p) => ({ ...p, facilityName: e.target.value }))}
            placeholder="St. Mary Medical Center"
            error={errors.facilityName}
          />
          <Input
            label="Contact Person"
            value={facForm.contactName}
            onChange={(e) => setFacForm((p) => ({ ...p, contactName: e.target.value }))}
          ...
          <Input
            ...
            placeholder="staffing@facility.org"
            error={errors.email}
          />
          <Input
            label="Password"
            ...
            onChange={(e) => setFacForm((p) => ({ ...p, password: e.target.value }))}
            placeholder="8+ characters"
            error={errors.password}
          />
          <Select
            label="Facility Type"
            value={facForm.facilityType}
            options={FACILITY_TYPES.map((t) => ({ value: t, label: t }))}
            placeholder="Select facility type"
            onChange={(e) => setFacForm((p) => ({ ...p, facilityType: e.target.value }))}
            error={errors.facilityType}
          />
          <Input
            label="Address"
            value={facForm.address}
            onChange={(e) => setFacForm((p) => ({ ...p, address: e.target.value }))}
            placeholder="123 Hospital Blvd, Los Angeles, CA"
          />
        </>
      )}
